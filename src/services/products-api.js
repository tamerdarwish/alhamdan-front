...
export const fetchProductById = async (id) => {
    try {
      const response = await fetch(`${process.env.REACT_APP_SERVER}/api/products/${id}`);
      if (!response.ok) {
...
export const adminFetchProducts = async (searchTerm) => {
  try {
    const response = await fetch(`${process.env.REACT_APP_SERVER}/api/products?search=${searchTerm}`);
    const data = await response.json();
