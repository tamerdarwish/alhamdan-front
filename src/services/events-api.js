...
export const fetchEventById = async (eventId) => {
  try {
    const response = await fetch(`http://26.206.131.69:5005/api/events/${eventId}`);
    const data = await response.json();
    return data.data
  } catch (error) {
...
export const handleDelete = async (id,onDelete) => {
  if (window.confirm('Are you sure you want to delete this event?')) {
    try {
      const response = await fetch(`http://26.206.131.69:5005/api/events/${id}`, {
        method: 'DELETE',
      });
...
export const verifyEventCode = async (eventCode) => {
  try {
    const response = await fetch(`http://26.206.131.69:5005/api/events/by-code/${eventCode}`);
    if (!response.ok) {
...
export const checkAccessCode = async (code) => {
  ...
  const response = await fetch(`http://26.206.131.69:5005/api/events/check-code/${code}`);
  const data = await response.json();
