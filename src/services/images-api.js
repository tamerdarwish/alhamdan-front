...
export const uploadImageToAlbum = async (eventId, formData) => {
  try {
      const response = await fetch(`https://alhamdan-back-8.onrender.com/api/upload/${eventId}/add-images`, {
          method: 'POST',
          body: formData,
        });
...
export const uploadImage = async (file) => {
  ...
  const uploadResponse = await fetch(`https://alhamdan-back-8.onrender.com/api/upload`, {
    method: 'POST',
    body: formData,
  });
...
export const downloadImagesWithWatermark = async (selectedImages,watermark_setting,eventId) => {
  ...
  try {
    for (let image of selectedImages) {
      ...
      const response = await fetch(`https://alhamdan-back-8.onrender.com/api/upload/watermark/${fileName}`, {
        method: 'POST',
        headers: {
...
export const deleteImageFromAlbum = async (eventId,imageId) => {
    try {
        const response = await fetch(`https://alhamdan-back-8.onrender.com/api/upload/${eventId}/delete-image`, {
            method: 'DELETE',
            headers: {
              'Content-Type': 'application/json'
            },
...
export const deleteSelectedImagesFromAlbum = async (eventId,selectedImages) => {
    try {
        const response = await fetch(`https://alhamdan-back-8.onrender.com/api/upload/${eventId}/delete-images`, {
            method: 'DELETE',
            headers: {
              'Content-Type': 'application/json'
            },
...
export const togglePrintStatus = async (eventId, imageId, currentStatus) => {
  try {
    const response = await fetch(`https://alhamdan-back-8.onrender.com/api/events/${eventId}/album/${imageId}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
...
export const uploadMainImage = async (eventId, file) => {
  ...
  try {
    const response = await fetch(`https://alhamdan-back-8.onrender.com/api/upload/update-main-image/${eventId}`, {
      method: 'POST',
